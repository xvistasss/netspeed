  ...
  if (isLocal && region !== "local-edge") {
    try {
      ...
      if (cfResponse.ok && cfResponse.body) {
        ...
        console.warn(
          `Cloudflare download fetch failed with status: ${cfResponse.status} ${cfResponse.statusText}`,
        );
        console.warn(
          "Headers:",
          Object.fromEntries(cfResponse.headers.entries()),
        );
        throw new Error(`Cloudflare fetch not ok: status ${cfResponse.status}`);
      }
    } catch (err: any) {
      console.error(
        "Failed to proxy remote download, falling back to local generator:",
        err.message || err,
      );
      // Fallback generator
